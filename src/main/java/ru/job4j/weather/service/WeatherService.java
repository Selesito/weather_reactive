...
public class WeatherService {
    ...
    public Mono<Weather> hottest() {
        int max = weathers.values().stream().mapToInt(Weather::getTemperature).max().getAsInt();
        return Mono.justOrEmpty(weathers.values().stream().filter(x -> x.getTemperature() == max).findFirst());
    }

    public Flux<Weather> cityGreatThen(int temp) {
        return Flux.fromIterable(weathers.values().stream().filter(x -> x.getTemperature() >= temp).collect(Collectors.toList()));
    }
}
